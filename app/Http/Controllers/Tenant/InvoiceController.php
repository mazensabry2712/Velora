...
class InvoiceController extends Controller
{
    ...
    /**
     * Return invoices belonging to the authenticated tenant customer account.
     * New booking-engine invoices reference customers.id, while authenticated
     * accounts are users, so email is the stable bridge between the two models.
     */
    ...
    private function recalculate(Invoice $invoice): Invoice
    {
        ...
        $invoice->update(['amount' => $subtotalCents / 100]);
        $invoice->refresh();
        return $invoice;
    }
