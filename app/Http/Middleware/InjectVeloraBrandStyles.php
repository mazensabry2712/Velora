...
class InjectVeloraBrandStyles
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        try {
            if (function_exists('tenant') && tenant() && \Illuminate\Routing\Route::has('password.request')) {
        ...
        // Highlight only the demo address text (not the entire browser address bar).
        $addressPattern = '~(<div[^>]*class="[^\"]*ms-3[^\"]*flex-1[^\"]*rounded-full[^\"]*"[^>]*>)(yourbusiness\.[^<]+)(</div>)~s';
        $content = preg_replace_callback(
            $addressPattern,
            static fn (array $matches): string => $matches[1]
                .'<span class="velora-address-label">'.e(trim($matches[2])).'</span>'
                .$matches[3],
            $content,
            1
        ) ?? $content;
