...
final class MasterBusinessFlowScenarioTest extends TenantTestCase
{
    ...
    public function business_rule_schema_matches_the_contract_used_by_the_booking_engine(): void
    {
        foreach (['key', 'value', 'type', 'description', 'is_active'] as $column) {
            $this->assertTrue(Schema::hasColumn('business_rules', $column), "business_rules.{$column} must exist for BusinessRule::getValue().");
        }
        ...
        BusinessRule::setValue(BusinessRule::MIN_ADVANCE_BOOKING_HOURS, 2, 'integer');
    ...
    public function appointment_status_history_schema_matches_the_model_contract(): void
    {
        ...
        foreach (['appointment_id', 'from_status', 'to_status', 'changed_by', 'actor_type', 'reason', 'metadata', 'created_at'] as $column) {
            $this->assertTrue(Schema::hasColumn('appointment_status_history', $column), "appointment_status_history.{$column} must exist for status history persistence.");
        }
    }
    ...
    public function golden_booking_flow_keeps_appointment_queue_customer_and_notification_state_consistent(): void
    {
        ...
        $response = $this->postJson('/api/appointments', [
            'customer_name' => 'QA Golden Customer', 'customer_email' => 'qa-golden@example.com', 'customer_phone' => '+201000000001',
            'appointment_date' => $date->toDateString(), 'appointment_time' => '09:00', 'staff_id' => $this->staffMember->id,
            'service_id' => $this->service->id, 'timezone' => $timezone, 'notes' => 'Golden scenario',
        ]);
        if ($response->status() !== 201) fwrite(STDERR, "Public booking response: {$response->getContent()}\n");
        ...
        $this->assertDatabaseHas('appointment_status_history', ['appointment_id' => $appointment->id, 'to_status' => Appointment::STATUS_PENDING]);
        $this->assertDatabaseHas('notification_deliveries', ['appointment_id' => $appointment->id, 'event' => 'appointment.booked', 'channel' => 'email', 'public_reference' => $appointment->public_reference]);
        $this->getJson('/api/queue/status/' . rawurlencode((string) $queue->queue_number))->assertOk()->assertJsonPath('success', true)->assertJsonPath('data.queue_number', $queue->queue_number)->assertJsonPath('data.status', 'waiting');
        $availability = $this->getJson('/api/booking/available-timeslots?' . http_build_query(['date' => $date->toDateString(), 'staff_id' => $this->staffMember->id, 'service_id' => $this->service->id, 'timezone' => $timezone]));
    ...
    public function dashboard_reconciles_exactly_with_database_truth_for_the_golden_dataset(): void
    {
        ...
        $pending = Appointment::create(['customer_id' => $this->customer->id, 'staff_id' => $this->staffMember->id, 'service_id' => $this->service->id, 'date' => $today, 'time_slot' => '09:00', 'status' => Appointment::STATUS_PENDING, 'price' => 100]);
        $confirmed = Appointment::create(['customer_id' => $this->customer->id, 'staff_id' => $this->staffMember->id, 'service_id' => $this->service->id, 'date' => $today, 'time_slot' => '10:00', 'status' => Appointment::STATUS_CONFIRMED, 'price' => 100]);
        $completed = Appointment::create(['customer_id' => $this->customer->id, 'staff_id' => $this->staffMember->id, 'service_id' => $this->service->id, 'date' => $today, 'time_slot' => '11:00', 'status' => Appointment::STATUS_PENDING, 'price' => 100]);
        $completed->update(['status' => Appointment::STATUS_COMPLETED]);
        Queue::create(['appointment_id' => $pending->id, 'queue_number' => 'A901', 'queue_date' => $today, 'status' => 'waiting', 'is_vip' => false]);
        Queue::create(['appointment_id' => $confirmed->id, 'queue_number' => 'A902', 'queue_date' => $today, 'status' => 'serving', 'is_vip' => true]);
        ...
        $this->assertSame(Appointment::whereDate('date', $today)->where('status', Appointment::STATUS_CONFIRMED)->count(), $stats['confirmed']);
        ...
        $this->assertSame(Appointment::where('service_id', $this->service->id)->count(), (int) $top->total);
    ...
    {
        $appointment = Appointment::create(['customer_id' => $this->customer->id, 'staff_id' => $this->staffMember->id, 'service_id' => $this->service->id, 'date' => today()->toDateString(), 'time_slot' => '14:00', 'status' => Appointment::STATUS_CONFIRMED]);
        $queue = Queue::create(['appointment_id' => $appointment->id, 'queue_number' => 'A950', 'queue_date' => today()->toDateString(), 'status' => 'waiting', 'is_vip' => false]);
        ...
        $this->assertDatabaseHas('invoices', ['appointment_id' => $appointment->id, 'customer_id' => $this->customer->id, 'status' => 'pending']);
    }
}
