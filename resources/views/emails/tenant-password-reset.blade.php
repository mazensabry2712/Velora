...
<div style="max-width:600px;margin:40px auto;padding:24px">
  <div style="background:#fff;border:1px solid #e5e7eb;border-radius:20px;padding:32px">
    ...
    <h1 style="margin:0 0 12px;font-size:28px">{{ __('password_reset.email_heading') }}</h1>
    <p style="font-size:15px;line-height:1.7">{{ __('password_reset.email_greeting', ['name' => $name]) }}</p>
    <p style="font-size:15px;line-height:1.7">{{ __('password_reset.email_body') }}</p>
    <p style="margin:28px 0"><a href="{{ $resetUrl }}" style="display:inline-block;background:#006cff;color:#fff;text-decoration:none;padding:14px 22px;border-radius:12px;font-weight:700">{{ __('password_reset.email_button') }}</a></p>
    <p style="font-size:12px;line-height:1.7;color:#667085">{{ __('password_reset.email_expiry') }}</p>
  </div>
</div>
</body></html>
