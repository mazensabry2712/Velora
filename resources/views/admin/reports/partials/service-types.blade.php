<div class="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
    <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4">{{ __('Service Types Report') }}</h3>
    @if($serviceTypes->isNotEmpty())
        <div class="space-y-3">
            @foreach($serviceTypes as $service)
                <div class="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-700 rounded">
                    <span class="text-slate-700 dark:text-slate-200">{{ $service->service_type ?? __('Not specified') }}</span>
                    <div>
