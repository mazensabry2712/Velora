            ...
            @foreach($staffPerformance as $staff)
                <div class="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-700 rounded">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 dark:text-slate-400">{{ $staff->role?->name ?? __('Staff') }}</p>
                    </div>
                    <div class="text-left rtl:text-right">
                        <span class="text-2xl font-bold text-slate-900 dark:text-slate-100">{{ $staff->staff_appointments_count }}</span>
