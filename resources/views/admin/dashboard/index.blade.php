...
@section('content')
        ...
        <div class="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-xl shadow-sm p-4 mb-6 text-white">
            <div class="flex items-center justify-between flex-wrap gap-4">
                ...
                <div class="flex items-center gap-4 text-sm">
                    <span title="{{ __('Users') }}">👥
                        {{ $subscriptionInfo['limits']['users']['current'] }}/{{ $subscriptionInfo['limits']['users']['max'] }}</span>
                    <span title="{{ __('Appointments This Month') }}">📅
                        {{ $subscriptionInfo['limits']['appointments']['current'] }}/{{ $subscriptionInfo['limits']['appointments']['max'] }}</span>
                    <a href="/admin/subscription"
                        class="px-4 py-2 bg-white text-indigo-600 rounded-lg font-medium hover:bg-white/90 transition-colors">
                        {{ __('Upgrade') }}
                    </a>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
            ...
            <div class="mt-4 flex items-center text-sm">
                ...
                @endphp
                <span
                    class="font-medium {{ $isPositive ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }}">
                    {{ $isPositive ? '+' : '' }}{{ $change }}%
                </span>
                <span
                    class="text-slate-500 dark:text-slate-400 {{ app()->getLocale() === 'ar' ? 'mr-2' : 'ml-2' }}">{{ __('from last week') }}</span>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
            ...
            <div class="mt-4 flex items-center text-sm">
                ...
                <span
                    class="text-slate-500 dark:text-slate-400 {{ app()->getLocale() === 'ar' ? 'mr-2' : 'ml-2' }}">{{ __('ready for today') }}</span>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
            ...
            <div class="mt-4 flex items-center text-sm">
                ...
                <span
                    class="text-slate-500 dark:text-slate-400 {{ app()->getLocale() === 'ar' ? 'mr-2' : 'ml-2' }}">{{ __('waiting now') }}</span>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-purple-600 dark:text-purple-400 mt-1">{{ $stats['customers'] ?? 0 }}
                    </p>
                </div>
            ...
            <div class="mt-4 flex items-center text-sm">
                <span
                    class="text-purple-600 dark:text-purple-400 font-medium">+{{ $stats['new_customers_this_week'] ?? 0 }}</span>
                <span
                    class="text-slate-500 dark:text-slate-400 {{ app()->getLocale() === 'ar' ? 'mr-2' : 'ml-2' }}">{{ __('new this week') }}</span>
            </div>
        </div>
    </div>
    ...
    <div
        class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border-2 border-blue-200 dark:border-blue-800 p-6 mb-8">
        <div class="flex items-start justify-between mb-4">
            <div>
                ...
                <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
                    {{ __('Quick overview of what needs your attention') }}</p>
            </div>
            <span
                class="text-2xl">{{ \Carbon\Carbon::now()->locale(app()->getLocale())->isoFormat('dddd, D MMMM') }}</span>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-slate-900 dark:text-slate-100">
                            {{ $stats['attendance_rate'] ?? 0 }}%</p>
                        <p class="text-xs text-slate-600 dark:text-slate-400">{{ __('Attendance rate today') }}</p>
                    </div>
                </div>
            </div>
            <!-- Cancellation Rate -->
            <div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-amber-200 dark:border-amber-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-slate-900 dark:text-slate-100">
                            {{ $stats['cancellation_rate'] ?? 0 }}%</p>
                        <p class="text-xs text-slate-600 dark:text-slate-400">{{ __('Cancellation rate this week') }}</p>
                    </div>
                </div>
            </div>
            <!-- Revenue This Month -->
            <div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-600 dark:text-slate-400">{{ __('Revenue this month') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <a href="/admin/appointments/create"
            class="group relative bg-gradient-to-br from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 rounded-xl shadow-lg hover:shadow-xl transition-all p-6 text-white overflow-hidden">
        ...
        <a href="/admin/queue"
            class="group relative bg-gradient-to-br from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 rounded-xl shadow-lg hover:shadow-xl transition-all p-6 text-white overflow-hidden">
            <div class="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-white/10 rounded-full"></div>
            <div class="relative">
                <div class="flex items-center justify-between mb-3">
                    <div class="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center">
        ...
        <a href="/admin/reports"
            class="group relative bg-gradient-to-br from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 rounded-xl shadow-lg hover:shadow-xl transition-all p-6 text-white overflow-hidden">
            <div class="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-white/10 rounded-full"></div>
            <div class="relative">
                <div class="flex items-center justify-between mb-3">
                    <div class="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 mb-8">
        <div class="border-b border-slate-200 dark:border-slate-700">
            <nav class="flex -mb-px">
                <button onclick="showTab('overview')" id="tab-overview"
                    class="tab-button active px-6 py-4 text-sm font-medium border-b-2 border-indigo-600 text-indigo-600 dark:text-indigo-400">
                    📊 {{ __('Overview') }}
                </button>
                <button onclick="showTab('distribution')" id="tab-distribution"
                    class="tab-button px-6 py-4 text-sm font-medium border-b-2 border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400 dark:hover:text-slate-300">
                    📈 {{ __('Distribution') }}
                </button>
        ...
        <div class="p-6">
            ...
            <div id="content-overview" class="tab-content">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100 mb-4">{{ __('Appointments Trend') }}
                    ({{ __('Last 7 Days') }})</h3>
                <div class="h-64">
            ...
            <div id="content-distribution" class="tab-content hidden">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100 mb-4">
                    {{ __('Appointment Status Distribution') }}</h3>
            ...
            <div id="content-team" class="tab-content hidden">
                <h3 class="font-semibold text-slate-900 dark:text-slate-100 mb-4">{{ __('Staff Performance') }}
                    ({{ __('This Month') }})</h3>
                <div class="space-y-4">
                    @if ($staffPerformance->isNotEmpty())
                        @foreach ($staffPerformance as $staff)
                            <div class="flex items-center gap-3">
                                ...
                                @else
                                    <div
                                        class="w-10 h-10 bg-purple-100 dark:bg-purple-900 rounded-full flex items-center justify-center flex-shrink-0">
                                        <span
                                            class="text-purple-600 dark:text-purple-400 font-bold">{{ substr($staff['name'], 0, 1) }}</span>
                                    </div>
                                @endif
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        ...
                                        <span
                                            class="text-xs text-slate-600 dark:text-slate-400">{{ $staff['completed'] }}/{{ $staff['total'] }}
                                            ({{ $staff['rate'] }}%)</span>
                                    </div>
                                </div>
                            </div>
                ...
                @if ($topServices->isNotEmpty())
                    ...
                    <div class="space-y-4">
                        @foreach ($topServices as $index => $service)
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 bg-indigo-100 dark:bg-indigo-900 rounded-lg flex items-center justify-center flex-shrink-0">
                                    <span
                                        class="text-indigo-600 dark:text-indigo-400 font-bold">#{{ $index + 1 }}</span>
                                </div>
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        <div class="flex-1 bg-slate-200 dark:bg-slate-700 rounded-full h-2">
                                            <div class="bg-indigo-600 h-2 rounded-full"
                                                style="width: {{ ($service->total / $maxTotal) * 100 }}%"></div>
                                        </div>
                                        <span
                                            class="text-sm font-medium text-slate-600 dark:text-slate-400">{{ $service->total }}</span>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="space-y-3">
                        @foreach ($recentActivities as $activity)
                            <div class="flex items-start gap-3 text-sm">
                                <div
                                    class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 {{ $activity['type'] === 'completed'
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-slate-500 dark:text-slate-400 text-xs">{{ $activity['customer'] }} •
                                        {{ $activity['time'] }}</p>
                                </div>
                            </div>
                        @endforeach
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 mb-8">
        <div class="p-5 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
            ...
            <a href="/admin/customers"
                class="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300">{{ __('View all') }}
                →</a>
        </div>
        <div class="divide-y divide-slate-100 dark:divide-slate-700">
            @if (isset($recentCustomers) && $recentCustomers->isNotEmpty())
                @foreach ($recentCustomers as $customer)
                    <div
                        class="flex items-center gap-4 px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors">
                        @if ($customer['avatar'])
                        ...
                        @else
                            <div
                                class="w-10 h-10 bg-purple-100 dark:bg-purple-900 rounded-full flex items-center justify-center flex-shrink-0">
                                <span
                                    class="text-purple-600 dark:text-purple-400 font-bold">{{ substr($customer['name'], 0, 1) }}</span>
                            </div>
                        @endif
                        ...
                        <div class="text-right flex-shrink-0">
                            <p class="text-sm font-medium text-slate-700 dark:text-slate-300">
                                {{ $customer['appointments_count'] }} {{ __('appts') }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                        ...
                        @foreach ($todayAppointments->take(5) as $appointment)
                            <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
                                <div
                                    ...
                                    <span
                                        class="text-indigo-600 dark:text-indigo-400 font-bold">{{ substr($appointment->customer?->name ?? '?', 0, 1) }}</span>
                                </div>
                                <div class="flex-1">
                                    <p class="font-medium text-slate-900 dark:text-slate-100">
                                        {{ $appointment->customer?->name ?? __('Unknown') }}</p>
                                    <p class="text-xs text-slate-500 dark:text-slate-400">
                                        {{ $appointment->service_name ?? ($appointment->service_type ?? 'N/A') }}</p>
                                </div>
                                <span
                                    class="text-sm text-slate-600 dark:text-slate-400">{{ $appointment->time_slot ?? 'N/A' }}</span>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-8">
                        ...
                        <a href="/admin/appointments"
                            class="text-indigo-600 dark:text-indigo-400 text-sm hover:underline mt-2 inline-block">{{ __('Create new appointment') }}</a>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
            <div class="p-5 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                ...
                <a href="/admin/queue"
                    class="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300">{{ __('Manage') }}
                    →</a>
            </div>
            <div class="p-5 max-h-96 overflow-y-auto">
                @if (isset($currentQueue) && $currentQueue->isNotEmpty())
                    <div class="space-y-3">
                        @foreach ($currentQueue->take(5) as $queue)
                            <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
                                <div
                                    ...
                                    <span
                                        class="text-amber-600 dark:text-amber-400 font-bold">#{{ $queue->queue_number ?? 0 }}</span>
                                </div>
                                <div class="flex-1">
                                    <p class="font-medium text-slate-900 dark:text-slate-100">
                                        {{ optional($queue->appointment)->customer?->name ?? ($queue->customer_name ?? __('Walk-in')) }}
                                    </p>
                                    <p class="text-xs text-slate-500 dark:text-slate-400">
                                        {{ ucfirst($queue->status ?? 'waiting') }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-8">
                        ...
                        <a href="/admin/queue"
                            class="text-indigo-600 dark:text-indigo-400 text-sm hover:underline mt-2 inline-block">{{ __('Add to queue') }}</a>
                    </div>
                @endif
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            new Chart(statusCtx, {
                type: 'doughnut',
                data: {
                    labels: ['{{ __('Pending') }}', '{{ __('Confirmed') }}', '{{ __('Completed') }}',
                        '{{ __('Cancelled') }}'
                    ],
                    datasets: [{
                        data: [
