...
<div class="fixed inset-0 pointer-events-none z-0 overflow-hidden">
    <div class="absolute -top-32 left-1/4 w-[700px] h-[500px] rounded-full blur-3xl" style="background:radial-gradient(ellipse,rgba(108,99,255,0.22) 0%,transparent 70%)"></div>
    <div class="absolute top-1/2 -right-48 w-[500px] h-[500px] rounded-full blur-3xl" style="background:rgba(56,189,248,0.06)"></div>
    <div class="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full blur-3xl" style="background:rgba(108,99,255,0.08)"></div>
</div>
...
<div class="relative z-10 min-h-screen flex pt-16 bg-surface">
    ...
    <div class="hidden lg:flex lg:w-[54%] flex-col justify-center px-12 xl:px-20 py-16 relative overflow-hidden">
        ...
        <div class="absolute top-0 left-0 w-[500px] h-[400px] rounded-full blur-3xl pointer-events-none" style="background:radial-gradient(ellipse,rgba(108,99,255,0.18) 0%,transparent 65%)"></div>
        <div class="absolute bottom-10 right-0 w-72 h-72 rounded-full blur-3xl pointer-events-none" style="background:rgba(56,189,248,0.07)"></div>
        ...
        <div class="relative z-10 max-w-lg">
            ...
            <div class="grid grid-cols-1 gap-3 mb-10">
                ...
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 text-base"
                         style="background:rgba(108,99,255,0.15);border:1px solid rgba(108,99,255,0.3)">{{ $b['icon'] }}</div>
            ...
            @endphp
            <p class="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-5">{{ __('landing.signup_what_next') }}</p>
            <div class="space-y-0">
                {{-- Step 1 --}}
                <div class="flex gap-4">
                    ...
                    <div class="pb-5">
                        <span class="text-xs text-brand-400 font-semibold">{{ __('landing.signup_timeline_1_label', ['days' => $maxTrialDays]) }}</span>
                        <p class="text-sm font-semibold text-white mt-0.5">{{ __('landing.signup_timeline_1_title') }}</p>
                        <p class="text-xs text-gray-500 mt-0.5 leading-relaxed">{{ __('landing.signup_timeline_1_desc') }}</p>
                    </div>
                </div>
                {{-- Step 2 --}}
                <div class="flex gap-4">
                    ...
                    <div class="pb-5">
                        <span class="text-xs text-yellow-400 font-semibold">{{ __('landing.signup_timeline_2_label', ['start' => $graceStart, 'end' => $graceEnd]) }}</span>
                        <p class="text-sm font-semibold text-white mt-0.5">{{ __('landing.signup_timeline_2_title') }}</p>
                        <p class="text-xs text-gray-500 mt-0.5 leading-relaxed">{{ __('landing.signup_timeline_2_desc') }}</p>
                    </div>
                </div>
                {{-- Step 3 --}}
                <div class="flex gap-4">
                    ...
                    <div>
                        <span class="text-xs text-gray-500 font-semibold">{{ __('landing.signup_timeline_3_label', ['day' => $roDay]) }}</span>
                        <p class="text-sm font-semibold text-white mt-0.5">{{ __('landing.signup_timeline_3_title') }}</p>
                        <p class="text-xs text-gray-500 mt-0.5 leading-relaxed">{{ __('landing.signup_timeline_3_desc') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 flex items-center gap-3">
                <div class="flex -space-x-2">
                    ...
                    <div class="w-9 h-9 rounded-full btn-primary flex items-center justify-center text-xs font-bold text-white border-2 border-surface">{{ $l }}</div>
                    @endforeach
                </div>
            ...
            <div class="glass rounded-3xl border border-brand-500/30 overflow-hidden"
                ...
                <div class="px-7 pt-7 pb-6 border-b border-white/5"
                    ...
                    <div class="flex items-center gap-3 mb-2">
                        <div class="w-10 h-10 rounded-2xl btn-primary flex items-center justify-center text-lg flex-shrink-0">🚀</div>
                        <div>
                            <h2 class="text-xl font-extrabold text-white leading-snug">
                                {{ __('landing.signup_title') }}
                            </h2>
                            <p class="text-xs text-gray-400 mt-0.5">{{ __('landing.signup_sub', ['days' => $maxTrialDays]) }}</p>
                        </div>
                    </div>
                    {{-- Mobile trust pills --}}
                ...
                <div class="px-7 py-7">
                    ...
                    <form id="signupForm" action="{{ route('signup.store') }}" method="POST" class="space-y-5" novalidate>
                        ...
                        <div x-data="{
                            ...
                            <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">{{ __('landing.signup_business_type') }}</label>

                            {{-- Hidden inputs that get submitted --}}
                            <input type="hidden" name="business_type" :value="selected === 'other' && custom.trim() ? custom.trim() : selected">
                            ...
                            <div class="grid grid-cols-4 gap-2">
                                ...
                                <button type="button" @click="pick('{{ $bt['val'] }}')"
                                        :class="selected === '{{ $bt['val'] }}' ? 'border-brand-500 bg-brand-500/10 text-white' : 'border-white/10 bg-white/5 text-gray-400 hover:border-brand-500/40 hover:text-gray-200'"
                                        class="flex flex-col items-center gap-1 rounded-xl py-3 px-1 border transition-all cursor-pointer">
                                    <span class="text-xl leading-none">{{ $bt['icon'] }}</span>
                                    <span class="text-[10px] leading-tight text-center">{{ __('landing.' . $bt['key']) }}</span>
                                </button>
                                @endforeach

                                {{-- Other / custom --}}
                                <button type="button" @click="pick('other')"
                                        :class="selected === 'other' ? 'border-brand-500 bg-brand-500/10 text-white' : 'border-white/10 bg-white/5 text-gray-400 hover:border-brand-500/40 hover:text-gray-200'"
                                        class="flex flex-col items-center gap-1 rounded-xl py-3 px-1 border transition-all cursor-pointer col-span-1">
                                    <span class="text-xl leading-none">✏️</span>
                                    <span class="text-[10px] leading-tight text-center">{{ __('landing.btype_other') }}</span>
                                </button>
                            </div>
                            ...
                            <div x-show="selected === 'other'" x-transition class="mt-2">
                                <input
                                    ...
                                    x-model="custom"
                                    placeholder="{{ __('landing.btype_other_ph') }}"
                                    maxlength="60"
                                    class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-gray-600 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500/50 transition-all text-sm"
                                />
                            </div>
                        ...
                        <div>
                            ...
                            <div class="flex rounded-xl overflow-hidden border border-white/10 focus-within:border-brand-500 focus-within:ring-1 focus-within:ring-brand-500/50 transition-all">
                                <input
                                    type="text"
                                    name="subdomain"
                                    id="subdomain"
                                ...
                                />
                                <span class="bg-white/[0.03] px-3 py-3 text-gray-500 text-xs font-mono border-l border-white/10 flex-shrink-0 flex items-center">
                                    .velora.com
                                </span>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">
                                    {{ __('landing.confirm_password') }} <span class="text-red-400 normal-case">*</span>
                                </label>
                                <input
                                    type="password"
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">{{ __('landing.country') }}</label>
                                <div class="relative">
                                    <select name="country"
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">{{ __('landing.dashboard_language') }}</label>
                                <div class="relative">
                        ...
                        <div x-data="{ open: {{ old('promo_code') ? 'true' : 'false' }} }">
                            ...
                            <div x-show="open" x-transition class="mt-2">
                                <input
                                    ...
                                    placeholder="{{ __('landing.signup_promo_ph') }}"
                                    maxlength="32"
                ...
                <div class="px-7 py-4 border-t border-white/5 text-center" style="background:rgba(255,255,255,0.02)">
                    <p class="text-sm text-gray-500">
                        ...
                        <a href="{{ route('central.login') }}" class="text-brand-400 hover:text-brand-300 font-semibold transition-colors">{{ __('landing.sign_in') }}</a>
                    </p>
                </div>
