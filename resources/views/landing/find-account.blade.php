            ...
            <div class="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] items-center">
                <div class="max-w-xl">
                    <a href="{{ route('landing') }}" class="inline-flex items-center gap-2 text-gray-400 hover:text-white text-sm mb-8 transition-colors group">
                        ...
                        {{ $workspace['back_to_home'] ?? ($isArabic ? 'العودة للرئيسية' : 'Back to home') }}
                    </a>
                    ...
                    <div class="mt-8 rounded-[2rem] border border-white/10 bg-white/[0.04] backdrop-blur-2xl p-8 shadow-2xl shadow-black/30">
                        ...
                        <div class="mt-6 pt-6 border-t border-white/10 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-gray-500">
                            <span class="inline-flex items-center gap-1.5"><svg class="w-3.5 h-3.5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>{{ $isArabic ? 'تشفير من طرف إلى طرف' : 'End-to-end encrypted' }}</span>
                            <span class="inline-flex items-center gap-1.5"><svg class="w-3.5 h-3.5 text-brand-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>{{ $isArabic ? 'دخول فوري' : 'Instant access' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
                        <p class="text-gray-400 text-sm">{{ $workspace['no_account'] }}
                            <a href="{{ route('signup') }}" class="text-brand-300 hover:text-brand-200 font-medium transition-colors">{{ $isArabic ? 'ابدأ التجربة المجانية' : 'Start free trial' }}</a>
                        </p>
                        <span class="hidden sm:inline text-gray-700">•</span>
                        <a href="{{ route('super-admin.login') }}" class="text-gray-400 hover:text-white text-sm transition-colors">{{ $isArabic ? 'دخول مسؤول النظام' : 'Super admin sign in' }}</a>
                    </div>
                </div>
                ...
                <div class="hidden lg:block">
                    <div class="relative rounded-[2rem] border border-white/10 bg-white/[0.03] backdrop-blur-xl p-6 shadow-2xl shadow-black/30 animate-float">
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-gray-500 mb-3 px-1">{{ $isArabic ? 'مساحات عمل نشطة' : 'Active workspaces' }}</p>
                        ...
                        <div class="mt-6 pt-5 border-t border-white/10 grid grid-cols-3 gap-3 text-center">
                            <div><p class="text-lg font-bold text-white">99.9%</p><p class="text-[11px] text-gray-500">{{ $isArabic ? 'وقت التشغيل' : 'Uptime' }}</p></div>
                            <div><p class="text-lg font-bold text-white">24/7</p><p class="text-[11px] text-gray-500">{{ $isArabic ? 'دعم فني' : 'Support' }}</p></div>
                            <div><p class="text-lg font-bold text-white">30+</p><p class="text-[11px] text-gray-500">{{ $isArabic ? 'دولة' : 'Countries' }}</p></div>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function redirectToLogin(e) {
        ...
        if(!subdomain||subdomain.length<3){errorEl.textContent=veloraFindAccount.messages.invalid;errorEl.classList.remove('hidden');return;}
    ...
    document.getElementById('subdomain').addEventListener('keydown',function(e){if(e.key==='Enter')document.getElementById('findAccountForm').dispatchEvent(new Event('submit'));});
</script>
@endsection
