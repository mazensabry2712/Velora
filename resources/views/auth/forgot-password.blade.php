...
<head>
    ...
    <style>
        .vpr-page{min-height:100dvh;background:var(--va-bg);color:var(--va-text);position:relative;overflow:hidden;display:flex;flex-direction:column}
        .vpr-page:before{content:"";position:absolute;width:650px;height:650px;top:-410px;inset-inline-start:-230px;border-radius:50%;background:radial-gradient(circle,rgba(109,70,255,.13),transparent 70%);pointer-events:none}
        .vpr-page:after{content:"";position:absolute;width:520px;height:520px;bottom:-350px;inset-inline-end:-220px;border-radius:50%;background:radial-gradient(circle,rgba(0,184,255,.10),transparent 70%);pointer-events:none}
        .vpr-shell{position:relative;z-index:1;width:min(1120px,calc(100% - 32px));min-height:100dvh;margin-inline:auto;display:flex;flex-direction:column;padding:18px 0 20px}
        .vpr-topbar{height:58px;display:flex;align-items:center;justify-content:space-between;gap:14px}
        .vpr-brand{display:inline-flex;align-items:center;gap:10px;color:inherit;text-decoration:none;min-width:0}
        ...
        .vpr-brand strong{display:block;max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:14px;font-weight:800;letter-spacing:-.02em}
        .vpr-brand small{display:block;margin-top:2px;color:var(--va-muted);font-size:9px;font-weight:700}
        .vpr-tools{display:flex;align-items:center;gap:7px}.vpr-tool{height:36px;min-width:36px;padding:0 10px;border:1px solid var(--va-line);border-radius:10px;background:var(--va-surface);color:var(--va-text);font-size:10px;font-weight:800;cursor:pointer;transition:.18s}.vpr-tool:hover{border-color:rgba(0,108,255,.35);color:var(--va-accent);transform:translateY(-1px)}
        ...
        .vpr-main{flex:1;display:flex;align-items:center;justify-content:center;padding:28px 0 30px}
        .vpr-card{width:min(100%,470px);border:1px solid var(--va-line);border-radius:25px;background:var(--va-surface);box-shadow:0 28px 80px rgba(13,18,38,.10);overflow:hidden}
        .vpr-accent{height:4px;background:linear-gradient(90deg,#6D46FF 0%,#006CFF 52%,#00B8FF 100%)}.vpr-inner{padding:31px}
        .vpr-identity{text-align:center}.vpr-logo{width:62px;height:62px;object-fit:contain;padding:8px;border:1px solid var(--va-line);border-radius:17px;background:var(--va-surface);box-shadow:0 10px 28px rgba(13,18,38,.07)}
        .vpr-name{margin-top:12px;color:var(--va-text);font-size:17px;font-weight:800;letter-spacing:-.02em}.vpr-domain{display:inline-flex;align-items:center;gap:7px;margin-top:6px;padding:6px 10px;border:1px solid var(--va-line);border-radius:999px;background:color-mix(in srgb,var(--va-surface) 94%,var(--va-bg));color:var(--va-muted);font-size:10px;font-weight:700;max-width:100%}.vpr-domain span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.vpr-domain svg{color:var(--va-accent);flex:0 0 auto}
        .vpr-heading{margin-top:25px;text-align:center}.vpr-heading h1{margin:0;color:var(--va-text);font-size:29px;line-height:1.12;letter-spacing:-.04em;font-weight:800}.vpr-heading p{margin:9px auto 0;max-width:330px;color:var(--va-muted);font-size:11px;line-height:1.75}
        .vpr-form{display:grid;gap:15px;margin-top:24px}.vpr-label{display:block;margin-bottom:8px;color:var(--va-text);font-size:10px;font-weight:800}.vpr-input-wrap{position:relative}.vpr-input-icon{position:absolute;top:50%;transform:translateY(-50%);inset-inline-start:14px;color:#8b95a7;pointer-events:none}.vpr-input{width:100%;height:53px;padding:0 43px;border:1px solid var(--va-line);border-radius:13px;background:color-mix(in srgb,var(--va-surface) 93%,var(--va-bg));color:var(--va-text);font-size:13px;outline:none;transition:.18s}.vpr-input::placeholder{color:#98a2b3}.vpr-input:focus{border-color:rgba(0,108,255,.58);box-shadow:0 0 0 4px rgba(0,108,255,.08);background:var(--va-surface)}
        ...
        .vpr-submit{width:100%;min-height:54px;border:0;border-radius:14px;background:linear-gradient(90deg,#6D46FF 0%,#006CFF 52%,#00B8FF 100%);color:#fff;font-size:12px;font-weight:900;cursor:pointer;box-shadow:0 14px 34px rgba(0,108,255,.19);transition:.18s}.vpr-submit:hover{transform:translateY(-1px);box-shadow:0 19px 42px rgba(0,108,255,.24)}
        ...
        .vpr-note{margin-top:18px;padding-top:15px;border-top:1px solid var(--va-line);text-align:center;color:#8792a5;font-size:9px;line-height:1.7}
        ...
        @media(max-width:600px){.vpr-shell{width:calc(100% - 20px);padding-top:10px}.vpr-brand img{width:36px;height:36px}.vpr-brand small{display:none}.vpr-main{padding:20px 0 22px}.vpr-inner{padding:23px 20px}.vpr-card{border-radius:22px}.vpr-heading h1{font-size:26px}.vpr-logo{width:58px;height:58px}}
        @media(max-width:400px){.vpr-brand strong{max-width:150px}.vpr-inner{padding:21px 17px}}
...
<div class="vpr-page">
    <div class="vpr-shell">
        <header class="vpr-topbar">
            <a href="{{ route('login') }}" class="vpr-brand" aria-label="{{ $displayName }}">
                ...
                <span>
                    ...
                    <small>{{ __('messages.login_to_account') }}</small>
                </span>
            </a>
            <div class="vpr-tools">
                ...
                <div class="vpr-inner">
                    <div class="vpr-identity">
                        ...
                        <div class="vpr-domain" title="{{ $tenantDomain }}">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3c2.6 2.6 3.8 5.6 3.8 9s-1.2 6.4-3.8 9c-2.6-2.6-3.8-6.4-3.8-9S9.4 5.6 12 3z"/></svg>
        ...
        <footer class="vpr-footer">
            <a href="{{ route('login') }}">{{ __('password_reset.back_to_login') }}</a> · Velora · {{ date('Y') }}
        </footer>
    </div>
</div>
