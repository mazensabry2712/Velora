...
<body
    class="bg-gradient-to-br from-slate-50 to-indigo-100 dark:from-slate-900 dark:to-slate-800 min-h-screen flex items-center justify-center p-4 transition-colors duration-300">
    ...
    <div class="absolute top-4 {{ $isArabic ? 'left-4' : 'right-4' }} flex items-center gap-3">
        ...
        <!-- Language Switcher -->
        <div
            class="inline-flex rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 p-1 shadow-sm">
    ...
    <div class="bg-white dark:bg-slate-800/95 w-full max-w-lg p-10 shadow-xl rounded-2xl dark:shadow-slate-900/50">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-4xl font-bold text-slate-800 dark:text-white mb-2">
                {{ $isArabic ? 'تسجيل الدخول' : 'Login' }}</h1>
        ...
        <form id="loginForm" class="space-y-5">
            ...
            <div>
                <label
                    class="block text-slate-700 dark:text-slate-200 font-medium mb-2">{{ $isArabic ? 'البريد الإلكتروني' : 'Email' }}</label>
            ...
            <div>
                <label
                    class="block text-slate-700 dark:text-slate-200 font-medium mb-2">{{ $isArabic ? 'كلمة المرور' : 'Password' }}</label>
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center cursor-pointer gap-2">
                    <input type="checkbox" id="remember"
                        class="w-4 h-4 rounded text-indigo-600 dark:bg-slate-700 dark:border-slate-600 focus:ring-indigo-500 dark:focus:ring-indigo-400">
                    <span class="text-slate-700 dark:text-slate-200">{{ $isArabic ? 'تذكرني' : 'Remember me' }}</span>
                </label>
                <a href="#"
                    class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 text-sm">{{ $isArabic ? 'نسيت كلمة المرور؟' : 'Forgot password?' }}</a>
            </div>

            <!-- Messages -->
            <div id="errorMessage"
                class="hidden bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 p-3 rounded-lg text-center">
            </div>

            <div id="successMessage"
                class="hidden bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 p-3 rounded-lg text-center">
            </div>
            ...
            <button type="submit" id="submitBtn"
                ...
                <svg class="hidden animate-spin w-6 h-6" id="loadingSpinner" xmlns="http://www.w3.org/2000/svg"
