<!DOCTYPE html>
@php
    // Laravel Localizer is the single source of truth for the current locale.
...
<footer class="v-footer"><div class="v-footer-inner"><img class="v-footer-ident" src="{{ asset('ident.png') }}" alt="Velora" /><div class="v-footer-meta">{{ __('landing.footer_copyright', ['year' => date('Y')]) }}</div></div></footer>
</div>
<script>
(function(){
    const r=document.documentElement,
        m=document.querySelector('meta[name="theme-color"]'),
        d=document.getElementById('themeToggleDesktop'),
        mo=document.getElementById('themeToggleMobile'),
        menu=document.getElementById('mobileMenu'),
        mt=document.getElementById('menuToggle');

    const i={
        sun:'<svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="4" stroke-width="1.8"/><path stroke-linecap="round" stroke-width="1.8" d="M12 2v2m0 16v2M4.93 4.93l1.42 1.42m11.3 11.3 1.42 1.42M2 12h2m16 0h2M4.93 19.07l1.42-1.42m11.3-11.3 1.42-1.42"/></svg>',
        moon:'<svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-width="1.8" d="M20.2 15.6A8.5 8.5 0 0 1 8.4 3.8 8.5 8.5 0 1 0 20.2 15.6Z"/></svg>'
    };
    ...
    function syncTheme(){
        ...
        if(m)m.content=dark?'#080B18':'#0D1226';
    }

    function toggleTheme(){
        const n=r.dataset.theme==='dark'?'light':'dark';
        r.dataset.theme=n;
        localStorage.setItem('velora-theme',n);
        syncTheme();
    }
    ...
    d?.addEventListener('click',toggleTheme);
    mo?.addEventListener('click',toggleTheme);
    syncTheme();
    ...
    document.addEventListener('click',function(e){
        if(!menu || !mt)return;
        if(menu.classList.contains('is-open') && !menu.contains(e.target) && !mt.contains(e.target)){
            setMenu(false);
        }
    });
