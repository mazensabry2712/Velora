(() => {
    ...
    const originalLabel = submit.tagName === 'INPUT'
        ? submit.value
        : submit.innerHTML;
    ...
    const setLabel = (message) => {
        if (submit.tagName === 'INPUT') {
            ...
            submit.innerHTML = `<span class="inline-flex items-center justify-center gap-2">` +
                `<span class="inline-block w-4 h-4 rounded-full border-2 border-white/40 border-t-white animate-spin" aria-hidden="true"></span>` +
