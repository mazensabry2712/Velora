    ...
    public function up(): void
    {
        Schema::table('country_settings', function (Blueprint $table) {
            $table->json('available_gateways')
                ...
                ->comment('Ordered list of payment gateways');
        });
        ...
        foreach ($defaults as $country => $gateways) {
            DB::table('country_settings')
                ->where('country_code', $country)
                ->update([
                    'available_gateways' => json_encode($gateways),
                ]);
        }

        DB::table('country_settings')
            ->whereNull('available_gateways')
            ->update([
                'available_gateways' => json_encode(['stripe']),
            ]);
    }
